    ...
    WITH    IDXC
              AS ( 		  
			    SELECT   *,
                   ...
                   FROM     ( SELECT    DISTINCT
                                        ...
                                        ,DENSE_RANK() OVER ( PARTITION BY TBL.[object_id] ORDER BY COALESCE(IDX.is_unique, 0) DESC, COALESCE(IDX.index_id,0) ) AS IndexPriority
             ...
             SELECT   TAB.[object_id] ,
                   ...
                   FROM     sys.objects TAB
                            ...
                            LEFT OUTER JOIN sys.indexes IDX ON ( TAB.[object_id] = IDX.[object_id] )
